...
ARG SDK_PATH=/usr/share/pico_sdk EXTRAS_PATH=/usr/share/pico_extras VERSION=2.2.0 USERNAME=devcontainer

RUN apt-get update &&\
    DEBIAN_FRONTEND=noninteractive apt-get install --no-install-recommends -y locales adduser git ca-certificates python3-venv build-essential cmake autoconf texinfo libtool libftdi-dev libusb-1.0-0-dev gcc-arm-none-eabi libnewlib-arm-none-eabi libstdc++-arm-none-eabi-newlib gdb-multiarch binutils-multiarch doxygen graphviz &&\
    apt clean &&\
